...
- **CSS inline (`build.inlineStylesheets: 'always'` en astro.config)**: el CSS va embebido en cada HTML — no hay request de stylesheet que cachear pero tampoco render-blocking (decisión PageSpeed 2026-07-21; el HTML gzippeado pesa igual que HTML+CSS separados). No revertir a `auto` sin medir con Lighthouse.
