...
- Regla nueva, gotcha o patrón para sesiones de IA → actualizar este `AGENTS.md`
...
1. **Corpus anti-alucinación y precedencia del cliente**: todo dato factual (servicios, tecnologías, certificaciones, capacidades, cifras) debe rastrearse a `content-source/`, al BrandBook o a materiales entregados/aprobados por el cliente. Los materiales más recientes del cliente son fuente de verdad y **prevalecen sobre el WordPress histórico**; cuando exista conflicto, aplicar el dato del cliente y actualizar `content-source/client-approved/` en el mismo cambio. PROHIBIDO inventar cifras que no aparezcan en alguna de estas fuentes.
2. Datos autorizados sin fuente adicional: más de 11 años de experiencia, experiencia en más de 6 países, datacenters TIER III y IV, CSIRT operativo, soporte 24/7, migración gratuita, sede Cll 121 #15a-50 Bogotá, tel +57 318 595 8261, tecnologías Acronis/Fortinet/FortiSIEM/Proxmox/VMware/Google Gemini/Mandiant/VirusTotal/Vicarius.
...
8. **Logos de organizaciones en Quiénes somos:** NO están autorizados para publicación. La presentación del cliente es material privado de referencia; la web solo puede mostrar el mapa regional y los 23 nombres textuales registrados en `content-source/client-approved/2026-07-ajustes-pagina-luma.md`.
...
- La URL/intención canónica sale de `docs/SEO-MATRIZ-2026.md`; `Plan_Maestro_pSEO_LumaCloud.docx` conserva el diagnóstico y volumen/KD histórico, que debe revalidarse antes de cada sprint
...
- **Analítica y leads (patrón obligatorio)**: eventos GA4 vía `src/lib/analytics.ts` — en elementos estáticos usar `data-track="<evento>"` + `data-track-<param>` (listener global en BaseLayout, agrega `page` y conserva el CTA); en scripts procesados importar `track()`/`appendAttribution()` de `@/lib/analytics`; en scripts inline (`define:vars`) usar `window.lumaTrack?.()` (no pueden importar módulos). Taxonomía de eventos en README §8 — no inventar nombres nuevos sin registrarlos ahí. Todo formulario nuevo de lead postea a `/api/contact` (con `source` propio + honeypot `website` + `appendAttribution`) y dispara `generate_lead` al éxito / `form_error` al fallo; el endpoint despacha a Resend y Zoho CRM en paralelo (cliente en `src/lib/zoho.ts`, env vars `ZOHO_*`).
- **Consentimiento en leads**: todo formulario nuevo incluye `consent=yes` obligatorio y `consent_version`; `appendAttribution()` añade el timestamp. La captura técnica no sustituye la validación legal del aviso/política. No quitar primer/último toque ni el contexto CTA/LCI que viaja a `/api/contact`.
- **pSEO controlado**: prohibido generar lotes de páginas por ciudad/sector. Seguir los gates de `docs/SEO-MATRIZ-2026.md`, pilotear máximo 3 URLs y medir 8–12 semanas antes de escalar.
- **Testimonios**: mantener visibles solo los cuatro verificados, pero no emitir Review schema autorreferencial ni prometer estrellas en Google.
