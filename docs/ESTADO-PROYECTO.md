...
- [x] **Segunda pasada** (mismo día, tras revisar el PageSpeed real): CSS inline en cada HTML (`build.inlineStylesheets: 'always'` — elimina el único request render-blocking; el HTML gzippeado queda igual o menor que HTML+CSS separados), recompresión de `home-cloud-secure-mobile.webp` (28→21,8KB) y `logo-luma-azul.webp` (10→7,8KB; el estimado de 2,6KB de PSI es teórico — el isotipo tiene degradados). Diagnóstico "unused JS" de gtag (68KB): aceptado, es post-`load` y sin puntuar. Lighthouse local: 99 sostenido, render-blocking en verde.
